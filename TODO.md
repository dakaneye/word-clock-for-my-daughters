...
- [ ] **Remove Cermant USB-C breakout from schematic + PCB** — decision locked: expose the ESP32 module's native micro-USB to the back panel via a USB-C-female to micro-USB-male panel-mount pigtail. Single user-facing USB-C port, data + power through the CP2102 bridge, no PCB respin needed to add USB data (just to remove J_USB1). Step-by-step in `docs/hardware/usb-c-breakout-removal-guide.md`. Updates `word-clock.kicad_sch`, `word-clock.kicad_pcb`, `word-clock-all-pos.csv`, `word-clock.csv`, gerbers.
- [ ] **Verify AITRIP module's USB VBUS input can carry system current** — visually inspect the micro-USB area for a polyfuse when parts arrive. If a 500 mA polyfuse is present, either bridge it, replace it, or add a separate power-only 5V input to the PCB before fab. See step 3.3 in the removal guide.
- [ ] **Generate 1:1 printable PCB overlay** — letter-sized PDF at 1:1 scale with letter grid cells (from face SVGs) + LED centers (from `hardware/word-clock-all-pos.csv`) + registration marks at known corners. Flag the documented (−1.5mm, −0.5mm) LED placement offset so it's visible.
- [ ] **Physical alignment check** — print overlay, lay against the finished face (once it arrives) and/or hold over the PCB print-out. Confirm each LED sits at the center of its target letter cell. Any systematic misalignment → PCB respin or grid spacing adjustment.
- [ ] **Measure bottom-side component heights** — read `hardware/word-clock.kicad_pcb` for tallest part (C2 bulk cap, ~16mm per spec). Drives back-panel standoff height.
