...
- **Back panel:** matching wood, engraved with dedication ("For Emory / love Dad" etc.). Carries 3 button access holes with "Hour / Min / Audio" labels next to each, a speaker vent grid, and one grommeted cable exit hole through which a permanently-installed Micro-USB-to-USB-C cable connects the ESP32 module's micro-USB port to the outside world. **Must be fully removable without damaging the wood, repeatably over 40+ years** — the DS3231 CR2032 coin cell needs replacement every 5-10 years. Attachment via M3 brass hex spacers structural-epoxied into each interior frame corner + countersunk brass machine screws through the panel. Corner placement bonds each spacer to two adjacent frame strips, sidestepping the 6.4 mm wall-thickness thin-wood problem that rules out direct threaded inserts. The DS3231 and HW-125 daughterboards must be installed with their battery holder / card slot facing the back panel (silkscreen markers on the main PCB enforce this) so the CR2032 and SD card remain accessible after removing the panel.
...
- **Diffuser:** white paper or frosted 0.5mm PETG behind the wood face, in front of the LEDs.
