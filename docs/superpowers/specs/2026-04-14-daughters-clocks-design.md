...
- **Back panel:** matching wood, engraved with dedication ("For Emory, love Dad, 2030" etc.). Carries the USB-C port (panel-mount pigtail to ESP32 micro-USB), 3 button access holes, speaker vent, and a small microSD slot cutout. **Must be fully removable without damaging the wood, repeatably over 40+ years** — the DS3231 CR2032 coin cell needs replacement every 5-10 years. Attachment planned via M3 brass threaded inserts in the frame interior + countersunk brass machine screws through the panel (see `TODO.md` for current decision status). The DS3231 and HW-125 daughterboards must be installed with their battery holder / card slot facing the back panel (silkscreen markers on the main PCB enforce this) so the CR2032 and SD card remain accessible after removing the panel.
