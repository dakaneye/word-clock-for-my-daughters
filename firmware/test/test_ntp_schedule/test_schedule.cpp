...
// Mid-curve N=2: 1 m = 60'000 ms (doubling of N=1).
void test_backoff_n2_one_minute(void) {
    TEST_ASSERT_EQUAL_UINT32(60'000u, next_backoff_ms(2));
}

// Mid-curve N=3: 2 m = 120'000 ms (doubling of N=2).
void test_backoff_n3_two_minutes(void) {
    TEST_ASSERT_EQUAL_UINT32(120'000u, next_backoff_ms(3));
}

// Mid-curve N=4: 4 m = 240'000 ms (doubling of N=3).
void test_backoff_n4_four_minutes(void) {
    TEST_ASSERT_EQUAL_UINT32(240'000u, next_backoff_ms(4));
}
...
// One past the first cap step: index clamp engages, cap holds.
void test_backoff_n7_still_at_cap(void) {
    TEST_ASSERT_EQUAL_UINT32(1'800'000u, next_backoff_ms(7));
}
...
int main(int, char**) {
    ...
    RUN_TEST(test_backoff_n2_one_minute);
    RUN_TEST(test_backoff_n3_two_minutes);
    RUN_TEST(test_backoff_n4_four_minutes);
    RUN_TEST(test_backoff_n5_eight_minutes);
    RUN_TEST(test_backoff_n6_at_cap);
    RUN_TEST(test_backoff_n7_still_at_cap);
